...
public class PropertiesPanel extends SmartScrollablePanel {
    ...
    public PropertiesPanel(Object propsObj, Field field, boolean showTypes, boolean autoApply) {
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            if (f.getX().isAnnotationPresent(StartSection.class)) {
                addSeparator(innerPanel, gbc, i + addI++, f.getX().getAnnotation(StartSection.class).value());
            }
            Class<?> cl = f.getX().getType();
            Component c;
            try {
                CompC c1 = checkClass(cl);
                c = c1.create(f.getY(), f.getX());
            } catch (NotSupportedClassException e) {
                Object o = f.getY();
                if (o == null) {
                    try {
                        o = f.getX().getType().getConstructor().newInstance();
                    } catch (InstantiationException | IllegalAccessException | InvocationTargetException |
                ...
                Object finalO = o;
                c = new PropertiesPanel(o, f.getX(), showTypes, autoApply);
                Component finalC = c;
                insideApply.add(() -> {
                    try {
                        f.getX().set(propsObj, finalO);
            ...
            addRow(innerPanel, gbc, i + addI, 0, c.getName() != null ? null : f.getX().getName(), c, showTypes ? f.getX().getType() : null);
            if (f.getX().isAnnotationPresent(Desc.class) && c.getName() == null) {
                addDesc(innerPanel, gbc, i + ++addI, f.getX().getAnnotation(Desc.class).value());
            }
            components.put(f.getX(), c);
        }
        ...
        toApply = () -> {
            for (Pair<Field, Object> f : fields) {
                try {
                    f.getX().set(propsObj, checkClass(f.getX().getType()).apply(components.get(f.getX())));
